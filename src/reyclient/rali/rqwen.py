...
class ClientORMTableAliQwen(ClientAli, rorm.Table):
    ...
    messages: rorm.Json = rorm.Field(rorm.JSONB, not_null=True, comment='Input messages data.')
    ...
    web: rorm.Json | None = rorm.Field(rorm.JSONB, comment='Web search data.')
...
class ClientAliQwen(ClientAli):
    ...
    def get_chat_records_history(
        ...
        for chat_record in chat_records_history_reverse:
            if chat_record['token'] is not None:
